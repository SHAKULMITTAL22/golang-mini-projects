...
func TestHandleGetBookById(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedStatus: http.StatusOK,
			expectedBody:   `{"id":"123","title":"Go Programming","author":"John Doe","price":"29.99","image_url":"http://example.com/image.jpg"}`,
		},
		{
			name:           "Book Not Found for a Given ID",
			bookId:         "999",
			expectedStatus: http.StatusOK,
		...
		{
			name:           "Internal Server Error Handling",
			bookId:         "500",
			mockError:      fmt.Errorf("database failure"),
			expectedStatus: http.StatusInternalServerError,
			expectedBody:   `{"Msg":"Internal server error"}`,
		},
		{
			name:   "Valid Book ID with Special Characters",
			bookId: "special!@#$%^&*()",
			mockResponse: Book{
				Id:       "special!@#$%^&*()",
			...
			expectedStatus: http.StatusOK,
			expectedBody:   `{"id":"special!@#$%^&*()","title":"Special Characters","author":"Jane Doe","price":"39.99","image_url":"http://example.com/special.jpg"}`,
		},
		{
			name:   "Book ID with Leading and Trailing Spaces",
			bookId: "  456  ",
			mockResponse: Book{
				Id:       "456",
				Title:    "Trim Spaces",
				Author:   "John Trim",
				Price:    "19.99",
				Imageurl: "http://example.com/trim.jpg",
			},
			expectedStatus: http.StatusOK,
			expectedBody:   `{"id":"456","title":"Trim Spaces","author":"John Trim","price":"19.99","image_url":"http://example.com/trim.jpg"}`,
		},
		{
			name:   "Large Book ID Input",
			bookId: strings.Repeat("A", 1000),
			mockResponse: Book{
				Id:       strings.Repeat("A", 1000),
				Title:    "Long ID",
				Author:   "Long Author",
				Price:    "49.99",
				Imageurl: "http://example.com/long.jpg",
			},
			expectedStatus: http.StatusOK,
			expectedBody:   fmt.Sprintf(`{"id":"%s","title":"Long ID","author":"Long Author","price":"49.99","image_url":"http://example.com/long.jpg"}`, strings.Repeat("A", 1000)),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			getBookById = func(id string) (Book, bool, error) {
				if tt.mockError != nil {
					return Book{}, false, tt.mockError
				}
				if id == strings.TrimSpace(tt.bookId) {
					return tt.mockResponse, true, nil
				}
				return Book{}, false, nil
			}

			req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/book?id=%s", tt.bookId), nil)
	...
	t.Run("Concurrent Requests for the Same Book ID", func(t *testing.T) {
		defer func() {
			if r := recover(); r != nil {
				t.Logf("Panic encountered, failing test. %v\n%s", r, string(debug.Stack()))
				t.Fail()
		...
		bookId := "123"
		expectedResponse := `{"id":"123","title":"Concurrency Test","author":"Concurrent Author","price":"59.99","image_url":"http://example.com/concurrency.jpg"}`
		getBookById = func(id string) (Book, bool, error) {
			return Book{
				Id:       "123",
				...
				Price:    "59.99",
				Imageurl: "http://example.com/concurrency.jpg",
			}, true, nil
		}
		...
		for i := 0; i < numRequests; i++ {
			...
			go func() {
				defer wg.Done()
				req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/book?id=%s", bookId), nil)
				w := httptest.NewRecorder()
				handleGetBookById(w, req)
				resp := w.Result()
				bodyBytes, _ := ioutil.ReadAll(resp.Body)
...
func TestJsonMessageByte(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: `{"Msg":"` + string(make([]byte, 1000)) + `"}`,
		},
		{
			name:     "Numeric String",
			input:    "12345",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if actual["Msg"] != expected["Msg"] {
				t.Errorf("Test '%s' failed. Expected: %s, Got: %s", tt.name, expected["Msg"], actual["Msg"])
			} else {
